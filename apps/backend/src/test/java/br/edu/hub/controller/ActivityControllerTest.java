...
class ActivityControllerTest {
    ...
    void shouldReturnDashboardMetrics() throws Exception {
        mockMvc.perform(get("/api/activities/dashboard/metrics"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.totalActivities").value(2))
                .andExpect(jsonPath("$.totalRegistrations").value(0))
                .andExpect(jsonPath("$.openActivities").value(1))
                .andExpect(jsonPath("$.fullActivities").value(1));
}
    @Test
    void shouldUpdateActivity() throws Exception {
        mockMvc.perform(
                patch("/api/activities/" + openActivity.getId())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                            {
                                "title": "Novo título",
